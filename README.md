...
- **Agentic Patterns** - Built-in components for tool calls, thinking, tasks, approvals, plans, and diffs
- **Tabbed Conversations** - Multi-tab support with `useTabChat` hook and `TabBar` component
- **Conversation Management** - Hooks for managing multiple conversations with pluggable persistence
...
- **Agent Status** - Real-time status spinner with elapsed time and stale detection
- **Extended Thinking** - Auto-expand/collapse thinking with duration tracking
- **Fully Themeable** - CSS variables for complete customization
- **TypeScript First** - Full type safety and great DX
- **Streaming Ready** - First-class support for streaming with partial message updates
...
| `thinkingDuration` | `number \| null` | Thinking elapsed time in seconds |
| `agentStatus` | `string \| null` | Current agent status (e.g., "Running tool_name...") |
...
import {
  ...
  ThinkingBox,         // Collapsible thinking/reasoning with auto-expand
  ToolCallCard,        // Tool execution with status
  TodoBox,             // Task progress list
  ApprovalCard,        // Permission request UI
  PlanCard,            // Plan approval with feedback
  DiffView,            // Code diff visualization
  QuestionCard,        // Multiple choice questions
  AgentStatusSpinner,  // Processing status with elapsed time
  TabBar,              // Multi-conversation tabs
...
<ThinkingBox
  thinking={text}
  isStreaming={true}        // Auto-expands during streaming, collapses when done
  duration={5}              // "Thought for 5s" shown when collapsed
  defaultCollapsed={true}   // Default state (overridden by streaming)
...
<PlanCard
  plan={{
    ...
    summary: 'Migrate database to support multi-tenancy.',
    steps: [
      { title: 'Create backup', toolName: 'pg_dump' },
      { title: 'Run migration', description: 'Add tenant_id column' },
      { title: 'Run tests', toolName: 'run_tests' },
    ],
  }}
  onApprove={() => {}}
  onReject={(feedback) => {}}  // Optional feedback string
/>

// AgentStatusSpinner - processing indicator with elapsed time
<AgentStatusSpinner
  status="Running analyze_data..."  // Current operation text
  onStop={() => {}}                 // Stop button (shown after warnAfter)
  showElapsedAfter={5}              // Show "(Xs)" after 5 seconds
  warnAfter={30}                    // Warning state after 30 seconds
/>

// TabBar - multi-conversation tab strip
<TabBar
  tabs={[
    { id: '1', label: 'Analysis', status: 'idle' },
    { id: '2', label: 'Refactoring', status: 'running' },
    { id: '3', label: 'Review', status: 'needs_input' },
  ]}
  activeTabId="1"
  onSelectTab={(id) => {}}
  onCloseTab={(id) => {}}
...
Run multiple conversations in parallel with `useTabChat` — a reducer-based hook that manages isolated tab state:

```tsx
import { useTabChat, TabBar, ChatContainer } from '@vith-ai/chat-ui'

function ChatApp() {
  const tabChat = useTabChat({
    adapter,
    maxTabs: 5,
    autoTitle: true,
  })
  ...
  return (
    <div className="flex flex-col h-full">
      <TabBar
        tabs={tabChat.tabs.map(t => ({ id: t.id, label: t.label, status: t.status }))}
        activeTabId={tabChat.activeTabId}
        onSelectTab={tabChat.selectTab}
        onCloseTab={tabChat.closeTab}
        onNewTab={() => tabChat.createTab()}
      />
      {tabChat.activeTab && (
        <ChatContainer
          messages={tabChat.activeTab.messages}
          isProcessing={tabChat.activeTab.isProcessing}
          thinkingText={tabChat.activeTab.thinkingText}
          thinkingDuration={tabChat.activeTab.thinkingDuration ?? undefined}
          agentStatus={tabChat.activeTab.agentStatus}
          tasks={tabChat.activeTab.tasks}
          pendingApproval={tabChat.activeTab.pendingApprovals[0]}
          diffs={tabChat.activeTab.diffs}
          error={tabChat.activeTab.error}
          onSend={tabChat.sendMessage}
          onStop={() => tabChat.stopProcessing()}
          onAnswerApproval={(approved) =>
            tabChat.answerApproval(tabChat.activeTab!.pendingApprovals[0].id, approved)
          }
        />
      )}
    </div>
...
Each tab is fully isolated — messages, approvals, questions, plans, tasks, diffs, and artifacts are scoped per tab. Tab status is computed automatically:

| Status | Meaning |
|--------|---------|
| `idle` | No pending work |
| `running` | Agent is processing |
| `needs_input` | Waiting for approval/question/plan response |
| `done` | Completed in background (while another tab was active) |
...
function ChatApp() {
  const conversations = useConversations({
    ...
    // store: createApiStore({ baseUrl: '/api/conversations' }),  // REST API
  })
