import { Send, Square, AlertCircle, X, RotateCcw } from 'lucide-react'
...
import { AgentStatusSpinner } from './AgentStatusSpinner'
...
export interface ChatContainerProps {
  ...
  /** Pending plan from assistant */
  pendingPlan?: PendingPlan
  /** Called when user responds to a plan */
  onAnswerPlan?: (approved: boolean, feedback?: string) => void
  /** Current agent status text (shown in spinner) */
  ...
  artifactRenderers?: Partial<Record<ArtifactType, (artifact: Artifact) => React.ReactNode>>
}
      ...
      <div className="flex-1 overflow-y-auto chat-scrollbar">
        {messages.length === 0 && (welcomeMessage || suggestions?.length) ? (
          ...
          <div className={clsx('py-4', centered && 'max-w-3xl mx-auto')}>
            ...
            {messages.map((message, index) => {
              ...
              return (
                <div key={message.id}>
                  ...
                  {!isStreamingMessage && (
                    <>
                      {/* Thinking for completed messages */}
                      {message.thinking && adapterFeatures?.thinking !== false && (
                      ...
                      />
                      {/* Artifacts */}
                      {showArtifacts && message.artifacts && message.artifacts.length > 0 && (
                        <div className="px-4 py-2">
                          <ArtifactPanel
                  ...
                  {isStreamingMessage && (
                    <>
                      {/* Agent status spinner - shown when no visible content yet */}
                      {!thinkingText && !message.toolCalls?.length && !message.content && (
                        <AgentStatusSpinner
                      ...
                      {/* Thinking box - only shown if adapter supports thinking */}
                      {thinkingText && adapterFeatures?.thinking !== false && (
                        <div className="px-4 py-2">
                          <ThinkingBox
            ...
            {/* Pending plan (shown after all messages) */}
            {pendingPlan && (
              <div className="px-4 py-2">
                <PlanCard
                  plan={pendingPlan}
                  onApprove={() => onAnswerPlan?.(true)}
                  onReject={(feedback) => onAnswerPlan?.(false, feedback)}
                  disabled={isProcessing}
                />
              </div>
            )}
